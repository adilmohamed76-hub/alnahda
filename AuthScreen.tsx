            ...
            <div className="bg-[var(--card-light)] dark:bg-[var(--card-dark)] rounded-2xl shadow-2xl p-8 w-full max-w-md" onClick={e => e.stopPropagation()}>
                {step === 1 && (
                    <form onSubmit={handleRequestCode}>
                        ...
                        <p className="text-center text-sm text-[var(--text-secondary-light)] dark:text-[var(--text-secondary-dark)] mb-6">أدخل بريدك الإلكتروني أو رقم هاتفك لإرسال رمز التحقق.</p>
                        {modalError && <p className="text-red-500 text-sm text-center mb-4">{modalError}</p>}
                        
                        <div className="flex justify-center gap-4 mb-4">
                            <button type="button" onClick={()=>setRecoveryMethod('email')} className={`px-4 py-2 rounded-lg ${recoveryMethod==='email' ? 'bg-blue-500 text-white' : 'bg-gray-200 dark:bg-gray-700'}`}>البريد الإلكتروني</button>
                            <button type="button" onClick={()=>setRecoveryMethod('phone')} className={`px-4 py-2 rounded-lg ${recoveryMethod==='phone' ? 'bg-blue-500 text-white' : 'bg-gray-200 dark:bg-gray-700'}`}>رقم الهاتف</button>
                        </div>

                        <div className="relative">
                            <span className="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-400">{recoveryMethod === 'email' ? <MailIcon /> : <PhoneIcon/>}</span>
                            <input type={recoveryMethod === 'email' ? 'email' : 'tel'} placeholder={recoveryMethod === 'email' ? 'بريدك الإلكتروني' : 'رقم هاتفك'} value={recoveryIdentifier} onChange={e => setRecoveryIdentifier(e.target.value)} required className="w-full pr-10 pl-4 py-3 text-sm rounded-lg"/>
                        </div>
                ...
                {step === 2 && (
                     <form onSubmit={handleVerifyCode}>
                        ...
                        <p className="text-center text-sm text-gray-500 mb-6">تم إرسال رمز مكون من 6 أرقام إلى {recoveryIdentifier}.</p>
                         {modalError && <p className="text-red-500 text-sm text-center mb-4">{modalError}</p>}
                        <div className="relative">
                            <span className="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-400"><KeyIcon/></span>
                            <input type="text" placeholder="أدخل الرمز" value={code} onChange={e => setCode(e.target.value)} required maxLength={6} className="w-full pr-10 pl-4 py-3 text-lg tracking-[0.5em] text-center font-mono rounded-lg"/>
                        </div>
                        <div className="flex gap-4 mt-6">
                ...
                {step === 3 && (
                     <form onSubmit={handleResetPassword}>
                        ...
                        <div className="relative">
                            ...
                            <input type="password" placeholder="كلمة المرور الجديدة" value={newPassword} onChange={e => setNewPassword(e.target.value)} required minLength={6} className="w-full pr-10 pl-4 py-3 text-sm rounded-lg"/>
                        </div>
                         <div className="flex gap-4 mt-6">
