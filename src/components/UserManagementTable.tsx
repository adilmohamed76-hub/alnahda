              ...
              <tr key={user.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                ...
                <td className="px-6 py-4 text-center">
                  <div className="flex justify-center items-center gap-2">
                    {hasPermission('MANAGE_USERS') && (
                      <>
                        <button onClick={() => handleOpenEditModal(user)} className="p-2 text-blue-500 hover:bg-blue-100 dark:hover:bg-blue-900/50 rounded-full" aria-label={`تعديل ${user.name}`}><PencilIcon /></button>
                        <button onClick={() => handleOpenDeleteConfirm(user)} className="p-2 text-red-500 hover:bg-red-100 dark:hover:bg-red-900/50 rounded-full" aria-label={`حذف ${user.name}`}><TrashIcon /></button>
                      </>
                    )}
                  </div>
                </td>
