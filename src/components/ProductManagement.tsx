...
import type { Product, Currency, Notification, Supplier, Warehouse, ProductCategory, Permission } from '../types/index';
                ...
                <React.Fragment key={product.id}>
                    <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td className="px-6 py-4 text-center no-print">
                            ...
                            <button onClick={() => handleOpenPrintModal(product)} className="p-2 text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-900/50 rounded-full" aria-label={`طباعة باركود لـ ${product.name}`}><BarcodeIcon /></button>
                            {hasPermission('MANAGE_PRODUCTS') && <button onClick={() => handleOpenEditModal(product)} className="p-2 text-blue-500 hover:bg-blue-100 dark:hover:bg-blue-900/50 rounded-full" aria-label={`تعديل ${product.name}`}><PencilIcon /></button>}
                            {hasPermission('DELETE_PRODUCTS') && <button onClick={() => handleOpenDeleteConfirm(product)} className="p-2 text-red-500 hover:bg-red-100 dark:hover:bg-red-900/50 rounded-full" aria-label={`حذف ${product.name}`}><TrashIcon /></button>}
                            </div>
                        </td>
                    </tr>
