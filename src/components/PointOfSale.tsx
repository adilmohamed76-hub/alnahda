      ...
      <div className="grid grid-cols-12 gap-6 flex-1 h-full">
        ...
        <div className="col-span-7 flex flex-col no-print">
            ...
            <div className="grid grid-cols-3 gap-2 mt-4 p-2 bg-gray-100 dark:bg-gray-900/50 rounded-lg">
                {numpadKeys.map(key => (
                    <button key={key} onClick={() => handleNumpad(key)} className="py-4 text-2xl font-bold bg-[var(--card-light)] dark:bg-[var(--card-dark)] rounded-md shadow hover:bg-gray-200 dark:hover:bg-gray-700" aria-label={key === 'del' ? 'حذف الرقم الأخير' : `الرقم ${key}`}>
                        {key === 'del' ? '⌫' : key}
                    </button>
                ))}
            </div>
            ...
            <div className="flex-1 overflow-y-auto">
                ...
                {items.map(item => (
                    <div key={item.productId} onClick={() => setSelectedItemId(item.productId)} className={`grid grid-cols-12 gap-2 items-center p-3 cursor-pointer border-r-4 ${selectedItemId === item.productId ? 'bg-blue-500/10 border-[var(--primary-color)]' : 'border-transparent'}`}>
                        ...
                        <div className="col-span-1 text-left"><button onClick={(e) => { e.stopPropagation(); setItems(prev => prev.filter(i => i.productId !== item.productId)); }} className="text-red-500 p-1 rounded-full hover:bg-red-100 dark:hover:bg-red-900/50" aria-label={`حذف ${item.productName}`}><TrashIcon className="w-4 h-4"/></button></div>
                    </div>
                ))}
