...
import { HomeIcon, CubeIcon, SalesIcon, PurchasesIcon, SettingsIcon, LogoutIcon, ShieldCheckIcon, TruckIcon, UsersIcon, WarehouseIcon, TagIcon, KeyIcon, CalculatorIcon, CashRegisterIcon, ArchiveIcon, ReportsIcon, ChevronDownIcon } from './Icons';
import type { View, User, Permission, SystemSettings } from './types';
...
const Sidebar: React.FC<SidebarProps> = ({ sidebarOpen, setSidebarOpen, onLogout, activeView, onNavigate, currentUser, hasPermission, systemSettings }) => {
  ...
  // Effect to open the dropdown containing the active link on initial load or navigation
  useEffect(() => {
  ...
  const renderLink = (link: NavLink) => {
    ...
    return (
        <a
          ...
          onClick={(e) => {
            e.preventDefault();
            if (link.view) handleLinkClick(link.view);
          }}
  ...
  return (
    <>
      ...
      <div
        className={`fixed inset-y-0 right-0 z-30 flex flex-col w-64 bg-[var(--card-light)] dark:bg-[var(--card-dark)] shadow-2xl transform transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0 ${
      ...
      >
        ...
        <div className="p-4 border-t border-[var(--border-light)] dark:border-[var(--border-dark)]">
           <a
              href="#"
              onClick={(e) => {
                e.preventDefault();
              ...
              className="flex items-center px-4 py-3 text-lg rounded-lg text-[var(--text-secondary-light)] dark:text-[var(--text-secondary-dark)] hover:bg-gray-500/5"
            >
              <LogoutIcon />
